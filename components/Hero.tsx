...
export default function Hero({ lang, dict }: { lang: Locale; dict: Dictionary }) {
  ...
  return (
    <section className="relative flex min-h-[88vh] items-end overflow-hidden bg-graphite-900">
      <Image
        src="/images/grey-arch/hero/hero-home.jpg"
        alt=""
        fill
        priority
        sizes="100vw"
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-6 pb-16 pt-44 lg:px-10">
        ...
        <AnimatedReveal delay={240}>
          <div className="mt-9 flex flex-wrap gap-4">
            <Link
              href={`/${lang}/services`}
              className="cursor-pointer bg-paper-100 px-6 py-3 text-sm text-graphite-900 transition-colors duration-200 hover:bg-bronze-400"
            >
              {hero.ctaExplore}
            </Link>
            <Link
              href={`/${lang}/portfolio`}
              className="cursor-pointer border border-paper-100/70 px-6 py-3 text-sm text-paper-100 transition-colors duration-200 hover:border-paper-100 hover:bg-paper-100/10"
            >
              {hero.ctaPortfolio}
            </Link>
            <Link
